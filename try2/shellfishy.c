...
int redirecting_left(char *args[], int index){
  int fd = open(args[index+1], O_RDWR | O_CREAT, 0666);
  if(fd==-1){
...
int redirecting_right(char *args[], int index){
  ...
  if(strcmp(args[index+2], ">")==0){
    int fd= open(args[index+3], O_RDWR | O_CREAT, 0666);
    if(fd == -1){
      printf("Redirection failed.\n");
      return 0;
    }
...
char ** temporary;
 ...
 while(1){
  ...
  char sub_command[500];
  strcpy(sub_command,trim(m_args[index]));
  
  int num_tokens = count_tokens(sub_command,' ');
